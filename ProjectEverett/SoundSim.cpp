...
void SoundSim::UpdateCameraPosition()
{
	...
	if (soundsCurrentlyPlaying && (cameraPtr = camera.lock().get()))
	{
		...
		const glm::vec3 listenerFront = cameraPtr->GetFrontVector();
		const glm::vec3 listenerUp = cameraPtr->GetUpVector();
...
{
	sound.playStates.SetStateChangeCallback(std::move(callback));
}
